...
export const seedDatabase = async () => {
  try {
    ...
    for (let i = 0; i < 25; i++) {
      ...
      const pricePerNight = faker.number.int({ min: 15000, max: 150000 });
      
      // Select random amenities
      const numAmenities = faker.number.int({ min: 5, max: 12 });
      ...
      properties.push({
        ...
        description: `${faker.lorem.paragraph(3)} This ${propertyType.toLowerCase()} is perfect for your stay in ${location.city}. ${faker.lorem.sentence()}`,
        propertyType,
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const nights = faker.number.int({ min: 2, max: 14 });
      const checkOutDate = new Date(checkInDate);
      checkOutDate.setDate(checkOutDate.getDate() + nights);
      
      const guestsCount = faker.number.int({ min: 1, max: property.maxGuests });
    ...
    const createdDestinations = await Destination.insertMany(TRENDING_DESTINATIONS);
    console.log(`✅ Created ${createdDestinations.length} destinations`);

    // Step 6: Create Deals
    console.log('🏷️  Creating deals...');
    const createdDeals = await Deal.insertMany(WEEKEND_DEALS);
